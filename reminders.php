...
if(global_reservation_reminders == '1')
{
	...
	{
		$query = mysqli_query("SELECT * FROM " . global_mysqli_users_table . " WHERE user_reservation_reminder='1'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysqli_error()) . '</span>');
		...
		while($user = mysqli_fetch_array($query))
		{
			...
			$query2 = mysqli_query("SELECT * FROM " . global_mysqli_reservations_table . " WHERE reservation_user_id='$user_id' AND reservation_week='$week_number' AND reservation_day='$day_number' ORDER BY reservation_time")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysqli_error()) . '</span>');
