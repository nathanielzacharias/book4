...
if(isset($_GET['make_reservation']))
{
	...
	echo make_reservation1($week, $day, $time);
}
...
elseif(isset($_GET['delete_reservation']))
{
	...
	$day = mysqli_real_escape_string($con,$_POST['day']);
	$time = mysqli_real_escape_string($con,$_POST['time']);
	echo delete_reservation1($week, $day, $time);
}
elseif(isset($_GET['read_reservation']))
{
	...
	$day = mysqli_real_escape_string($con, $_POST['day']);
	$time = mysqli_real_escape_string($con, $_POST['time']);
	echo read_reservation1($week, $day, $time);
}
elseif(isset($_GET['read_reservation_details']))
{
	...
	$day = mysqli_real_escape_string($con,$_POST['day']);
	$time = mysqli_real_escape_string($con,$_POST['time']);
	echo read_reservation_details1($week, $day, $time);
}
elseif(isset($_GET['week']))
{
	...
	foreach($global_times as $time)
	{
		...
		while($i < 7)
		{
			...
			echo '<td><div class="reservation_time_div"><div class="reservation_time_cell_div" id="div:' . $week . ':' . $i . ':' . $time . '" onclick="void(0)">' . read_reservation1($week, $i, $time) . '</div></div></td>';
		}
